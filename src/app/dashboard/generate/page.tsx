...
import {
  ...
  BookMarked, ArrowRight, ChevronDown, Zap, Info,
  CheckCircle2, DollarSign as SellIcon
} from 'lucide-react'
...
function useLoadingStep(loading: boolean) {
  ...
  useEffect(() => {
    ...
    setStep(0)
    const iv = setInterval(() => setStep(p => p < LOADING_STEPS.length - 1 ? p + 1 : p), 1800)
...
function ScoreBar({ score, label, icon }: { score: number; label: string; icon: React.ReactNode }) {
  const color = score >= 75 ? 'from-indigo-500 to-indigo-600'
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className="text-[11px] font-bold text-slate-600 w-6 text-right tabular-nums">{score}</span>
    </div>
  )
}
...
export default function GeneratePage() {
  ...
  const handleGenerate = async () => {
    if (loading) return
    setLoading(true); setError(''); setSaveError(null); setIdeas([]); setSavedIds(new Set())
    try {
  ...
  const handleSave = async (idea: Idea, index: number) => {
    ...
    setSavingId(index); setSaveError(null)
    try {
    ...
    <div className="flex w-full min-h-screen bg-[#f5f6fa]">
      ...
      <main className="flex-1 md:ml-60 px-4 md:px-8 pt-20 md:pt-10 pb-16">
        ...
        <div className="mb-7">
          ...
          <p className="text-slate-400 text-sm mt-1">Discover trending digital product angles with AI-powered scoring</p>
        </div>

        {/* ── Input Card ── */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 md:p-6 mb-8">
          <p className="text-xs font-black text-slate-500 uppercase tracking-widest mb-5">Configure Your Forge</p>

          {/* Topic */}
          <div className="mb-4">
            <label className="block text-xs font-bold text-slate-600 mb-2">
              Topic <span className="text-slate-400 font-normal normal-case">(optional — leave blank for AI picks)</span>
            </label>
            <input
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-2">Niche <span className="text-red-400">*</span></label>
  ...
  <div className="mt-3 flex items-center gap-2 text-[11px] text-indigo-600 bg-indigo-50 border border-indigo-100 rounded-lg px-3 py-2">
    ...
    <span className="font-medium">🔥 General mode = Only high-potential, money-making ideas (75+ score minimum)</span>
  </div>
)}
<br></br>
        ...
        {saveError && (
          <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl p-4 mb-6 text-sm">
            ...
            <div><p className="font-black mb-0.5">Save failed</p><p>{saveError}</p></div>
          </div>
        )}
        ...
        {!loading && ideas.length > 0 && (
          <>
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-base font-black text-slate-900">{ideas.length} Ideas Ready</h2>
                <p className="text-xs text-slate-400 mt-0.5">Ranked by Forge Score · highest first</p>
              </div>
                    ...
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <button
                        ...
                        className={`shrink-0 p-1.5 rounded-xl transition cursor-pointer ${
                          savedIds.has(i) ? 'text-indigo-600 bg-indigo-50' :
                          savingId === i ? 'text-indigo-400 bg-indigo-50 animate-pulse' :
                      ...
                      >
                        ...
                        ) : savedIds.has(i) ? <CheckCircle2 className="w-4 h-4" /> : <BookMarked className="w-4 h-4" />}
                      </button>
                    </div>

                    {/* Title + audience */}
                    <h3 className="text-sm font-black text-slate-900 leading-snug mb-1">{idea.title}</h3>
                    <p className="text-slate-500 text-xs leading-relaxed mb-3">{idea.angle}</p>
                    <p className="text-[11px] text-slate-400 mb-3">👤 {idea.targetAudience}</p>

                    {/* ── Why it sells ── */}
                    {idea.whyItSells && (
                      <div className="flex items-start gap-2.5 bg-emerald-50 border border-emerald-100 rounded-xl px-3.5 py-2.5 mb-4">
                        <span className="text-emerald-500 shrink-0 mt-0.5">💰</span>
                        <p className="text-[11px] text-emerald-800 font-medium leading-relaxed">{idea.whyItSells}</p>
                      </div>
                    )}
  ...
  <div className="mb-4">
    ...
    <div className="flex flex-wrap gap-2">
      {idea.searchTerms.map((term: string, idx: number) => (
        <span key={idx} className="text-[10px] font-medium bg-white border border-slate-200 text-slate-600 px-2.5 py-1 rounded-full shadow-sm">
          {term}
        </span>
      ))}
    </div>
                    ...
                    <div className="space-y-2 p-3 bg-slate-50 rounded-xl border border-slate-100 mb-4">
                      <ScoreBar score={idea.demandScore}       label="Demand"       icon={<TrendingUp className="w-3.5 h-3.5" />} />
                      <ScoreBar score={idea.competitionScore}  label="Competition"  icon={<Sword className="w-3.5 h-3.5" />} />
                      <ScoreBar score={idea.monetizationScore} label="Monetization" icon={<DollarSign className="w-3.5 h-3.5" />} />
                      <ScoreBar score={idea.viralityScore}     label="Virality"     icon={<Flame className="w-3.5 h-3.5" />} />
                    </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-400">
                            {idea.forgeScore >= 75 ? '🔥 High potential' : idea.forgeScore >= 50 ? '👍 Worth trying' : '⚠️ Lower priority'}
                          </p>
                        </div>
                      </div>
                      <button onClick={() => handleForge(idea)}
        ...
        {!loading && ideas.length === 0 && !error && (
          <div className="text-center py-20">
            ...
            <p className="text-slate-400 text-sm max-w-xs mx-auto">
              Pick a niche and hit "Forge Ideas" to discover high-potential digital product angles.
            </p>
          </div>
        )}
