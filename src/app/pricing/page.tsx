...
export default function PricingPage() {
  ...
  const comparisonFeatures = [
    {
      name: "Monthly ebook generations",
      free: PLANS.free.limits.ebookGenerationsPerMonth,
      starter: PLANS.starter.limits.ebookGenerationsPerMonth,
      pro: PLANS.pro.limits.ebookGenerationsPerMonth,
    },
    {
      name: "Max chapters per ebook",
      free: PLANS.free.limits.maxChapters,
      starter: PLANS.starter.limits.maxChapters,
      pro: PLANS.pro.limits.maxChapters,
    },
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-2xl border-b border-white/[0.05] bg-[#0f1117]/80">
        <div className="flex items-center justify-between px-4 md:px-6 py-4 md:py-5 max-w-7xl mx-auto">
          ...
          <div className="hidden lg:flex items-center gap-6 xl:gap-10 text-xs font-bold uppercase tracking-widest text-zinc-500">
            ...
            <Link href="/pricing" className="text-indigo-400 transition">
              Pricing
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center gap-4">
            <Link
              href="/login"
              className="text-xs font-bold text-zinc-400 hover:text-white transition uppercase tracking-widest"
            >
              Login
            </Link>
            <Link
              href="/signup"
              className="bg-white text-black text-xs font-black px-5 py-2.5 rounded-full hover:bg-indigo-50 transition shadow-xl uppercase tracking-widest"
            >
              Sign Up Free
            </Link>
          </div>
          ...
          <div className="lg:hidden bg-[#0f1117]/95 backdrop-blur-2xl border-t border-white/[0.05] px-4 py-4">
            <div className="flex flex-col gap-4">
              ...
              <Link
                href="/pricing"
                onClick={() => setIsMobileMenuOpen(false)}
                className="text-sm font-bold text-indigo-400 transition py-2"
              >
                Pricing
              </Link>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 pb-16 md:pb-24">
          ...
          {plansForDisplay.map((plan) => {
            ...
            return (
              ...
              >
                {plan.badge && (
                  <div
                    className={`absolute -top-3 left-1/2 -translate-x-1/2 text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] px-3 md:px-4 py-1 md:py-1.5 rounded-full flex items-center gap-1 whitespace-nowrap ${
                      plan.highlight
                ...
                {isCurrentPlan && (
                  <div className="absolute top-3 right-3 md:top-4 md:right-4 bg-emerald-500/20 border border-emerald-500/30 rounded-full px-2 py-0.5">
                    <span className="text-[8px] md:text-[9px] font-black text-emerald-400 uppercase">
                      Current
                    </span>
                  </div>
                )}
                ...
                <div className="mb-4 md:mb-6">
                  <p
                    className={`text-[11px] md:text-xs font-black uppercase tracking-[0.25em] mb-2 md:mb-3 ${plan.highlight ? "text-indigo-100" : "text-zinc-500"}`}
                  >
                    {plan.name}
                  </p>
                  <div className="flex items-end gap-1 mb-2">
                    <span
                      className={`text-3xl md:text-4xl lg:text-5xl font-black tracking-tighter ${plan.highlight ? "text-white" : "text-white"}`}
                    >
                      {isFree ? "Free" : `$${plan.price}`}
                    </span>
                    {!isFree && (
                      <span
                        className={`text-xs md:text-sm font-medium mb-1 md:mb-2 ${plan.highlight ? "text-indigo-100" : "text-zinc-600"}`}
                      >
                        /month
                      </span>
                    )}
                  ...
                  <p
                    className={`text-xs md:text-sm ${plan.highlight ? "text-indigo-100" : "text-zinc-500"}`}
                  >
                    {plan.description}
                  </p>
                </div>
                ...
                {isCurrentPlan ? (
                  ...
                  <button
                    onClick={() => handleSubscribe(plan.id, plan.planCode!)}
                ...
                <div
                  className={`h-px mb-4 md:mb-6 ${plan.highlight ? "bg-white/20" : "bg-white/[0.06]"}`}
                />
                ...
                <ul className="space-y-2 md:space-y-3">
                  {plan.features.map((feat, j) => (
                    <li key={j} className="flex items-start gap-2 md:gap-2.5">
                      <Check
                        className={`w-3.5 h-3.5 md:w-4 md:h-4 shrink-0 mt-0.5 ${plan.highlight ? "text-white" : "text-indigo-400"}`}
                      />
                      <span
                        className={`text-[11px] md:text-xs ${plan.highlight ? "text-white" : "text-zinc-400"}`}
                      >
                        {feat}
                        {feat === "AI cover image generation" && (
                          <span className="ml-1.5 text-[8px] bg-amber-500/20 text-amber-400 px-1.5 py-0.5 rounded-full">
                            Soon
                          </span>
                        )}
                      </span>
        ...
        <div className="text-center mt-10 md:mt-12">
          ...
          >
            ...
            <ArrowRight
              className={`w-3 h-3 md:w-3.5 md:h-3.5 transition-transform ${showComparison ? "rotate-90" : ""}`}
            />
          </button>
        </div>
          ...
          <div className="mt-6 md:mt-8 bg-zinc-900/40 rounded-xl md:rounded-2xl border border-white/[0.07] overflow-x-auto">
            <div className="min-w-[640px] md:min-w-0">
              <div className="p-4 md:p-6 border-b border-white/[0.07]">
                <h3 className="font-black text-white text-center text-base md:text-lg">
              ...
              <table className="w-full text-xs md:text-sm">
                <thead>
                  <tr className="border-b border-white/[0.07] bg-white/[0.03]">
                    <th className="text-left p-3 md:p-4 font-bold text-zinc-400">
                      Feature
                    </th>
                    <th className="text-center p-3 md:p-4 font-bold text-zinc-400 w-20 md:w-28">
                      Free
                    </th>
                    <th className="text-center p-3 md:p-4 font-bold text-indigo-400 w-20 md:w-28 bg-indigo-600/5">
                      Starter
                    </th>
                    <th className="text-center p-3 md:p-4 font-bold text-zinc-400 w-20 md:w-28">
                      Pro
                ...
                <tbody>
                  {comparisonFeatures.map((row, i) => (
                    ...
                    >
                      <td className="p-3 md:p-4 font-medium text-zinc-300 text-[11px] md:text-sm">
                        {row.name}
                      </td>
                      <td className="text-center p-3 md:p-4 text-zinc-500 text-[11px] md:text-sm">
                        {row.free}
                      </td>
                      <td className="text-center p-3 md:p-4 text-indigo-300 font-medium bg-indigo-600/5 text-[11px] md:text-sm">
                        {row.starter}
                      </td>
                      <td className="text-center p-3 md:p-4 text-zinc-500 text-[11px] md:text-sm">
                        {row.pro}
